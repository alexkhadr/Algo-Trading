...
On the third provided dataset, the final strategy achieved a Sharpe ratio of 0.1824 and an average turnover of 0.0535 using the project evaluation methodology. These results, combined with the strategy's theoretical robustness properties — particularly its independence from unrestricted sample covariance inversion and its scalability to varying asset universe sizes — motivated its selection as the final submission.

The primary limitation of the approach is its reliance on factor-model expected returns, which remain susceptible to estimation noise at monthly frequency despite Ridge regularization. Future extensions could explore shrinkage of the return vector itself (for example, through a James-Stein or Black-Litterman estimator), regime-switching components that adjust the model structure during periods of market stress, or ensemble approaches that combine Ridge + LW with the turnover-stability of risk parity.
